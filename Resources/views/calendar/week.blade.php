<div class="week-view">
    ...
    <div class="week-sticky">
        ...
        {{-- All-day row --}}
        <div class="week-row week-allday">
            <div class="time-slot-label">ALL DAY</div>
            @foreach($weekDays as $day)
                <div class="week-allday-cell {{ $day['isToday'] ? 'today' : '' }}">
                    @foreach($day['events']->where('all_day', true) as $event)
                        @php
                        ...
                        @endphp
                        <a href="#"
                           class="week-allday-event view-event-btn"
        ...
        @foreach($timeSlots as $slotIndex => $time)
            <div class="week-row">
                ...
                @foreach($weekDays as $dayIndex => $day)
                    <div class="time-slot-cell {{ $day['isToday'] ? 'today' : '' }}">
                        @if($slotIndex === 0)
                            {{-- Calculate overlaps for all events in this day --}}
                            @php
                                $dayEventsArray = $day['events']->where('all_day', false)->values()->all();
                                $eventPositions = [];
                            ...
                            @foreach($day['events']->where('all_day', false)->values() as $eventIndex => $event)
                                @php
